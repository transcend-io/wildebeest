...
export default class MigrationLock extends Model {
   ...
   */
  public static async runWithLock(
    ...
    if (CHECK_SYNC) {
      await testSetup(this.sequelize, models);
    }
  ...
  public async latest(schema = RESTORE_ON_EMPTY): Promise<void> {
    ...
    await clearSchema(this.sequelize.getQueryInterface());

    // Restore the db to the latest save
    await restoreFromDump(wildebeest, schema);
